...
/* ------------------------------------------------------------------------------
 *  Build instructions:
 *  1:  Before installing the board support, add the following URL in the Boards
 *      Manager List:
 *      https://mcudude.github.io/MightyCore/package_MCUdude_MightyCore_index.json,https://github.com/ms-iot/iot-utilities/raw/master/IotCoreAppDeployment/ArduinoIde/package_iotcore_ide-1.6.6_index.json
 *  2:  Start the Boards Manager, search for MightyCore and install it.
 *  3:  Select ATmega32 in the Board List under MightyCore
 *  4:  Settings:
 ...
 *      - Pinout:       Standard
 *      - Bootloader:   Yes
 *  5:  No external library needed
 *  6:  Connect the Z80-MBC2 board
 *  7:  Select the comms port
 *  8:  Build and upload.
 *  9:  Immediately after "Uploading" appears in the build/upload status bar above
 *      the output pane, press the Reset button.
 *  10: Serial Monitor speed: 115200
------------------------------------------------------------------------------ */


#define GME_EXT     1       // Enable GmEsoft Extensions
#define GME_KEYB    1       // Enable Keyboard Mapping Switch
...
const byte    maxDiskSet   = 7;           // Number of configured Disk Sets
#if GME_EXT
const word    trackMax     = 4095;        // Max number of tracks in disk images
#endif
...
#if GME_KEYB
const byte  keyb_be[] PROGMEM =          // Belgian keyboard mapping for UTERM
    "\x00\x11\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F"
    "\x10\x01\x12\x13\x14\x15\x16\x1A\x18\x19\x17\x1B|~\x1E\x1F"
    " 1%3457`908_;):="
    "@&[\"'(#]!^Mm.-/+"
    "2QBCDEFGHIJKL?NO"
    "PARSTUVZXYW{<$6\\"
...
byte          Z80IntEnFlag   = 0;         // Z80 INT_ enable flag (0 = No INT_ used, 1 = INT_ used for I/O) [DEPRECATED]
...
#if GME_KEYB
byte          lastEsc        = 0;         // Last escape sequence char
const byte *  kbMap          = 0;         // Keyboard mapping table pointer
#endif
...
// ------------------------------------------------------------------------------
  ...
  if ( !digitalRead(USER) )
  {
    bootSelection = 1;
    timeStamp = millis();
    while ( millis() - timeStamp < 5000 && !digitalRead(USER) ) ;
    if ( !digitalRead(USER) )
    {
      for(;;)
      {
...
#if GME_EXT
  Serial.println(F("IOS: ** GmEsoft extensions - build: " __DATE__ " " __TIME__ " **\r\n"));
#endif
  ...
  switch (bootMode)
  {
    case 0:                                       // Load Basic from SD
      ...
      Z80IntEnFlag = 1;                           // Enable INT_ signal generation (Z80 M1 INT I/O) [DEPRECATED]
    ...
    case 2:                                       // Load an OS from current Disk Set on SD
      ...
      case 5:                                     // FUZIX ********************************************************
        ...
        Z80IntEnFlag = 3;                           // Enable INT_ signal generation (Z80 M1 INT I/O) [DEPRECATED]
        Z80IntRx = 1;                             // Enable Z80 Rx INT_ signal generation (Z80 M1 INT I/O)
      ...
      break;
        ...
        fileNameSD = FUZIXFN;
        BootStrAddr = FUZSTRADDR;
        Z80IntEnFlag = 0;                           // Enable INT_ signal [DEPRECATED]
...
void loop() 
{
    ...
    
    {
...
#if GME_EXT
      // Opcode 0x60  SELDISKSET      1 (0-2,...)
      // Opcode 0x62  SETINTENABLE    1 (0=no,1=yes) [DEPRECATED]
...
#if GME_EXT
      // Opcode 0xE0  GETDISKSET      1 (0-2,...)
      // Opcode 0xE2  GETINTENABLE    1 (0=no,1=yes) [DEPRECATED]
      ...
      //
      {
...
#if GME_EXT
            if ((trackSel < trackMax) && (sectSel < 32))
#else
        ...
        case  0x0C:
          ...
          {
#if GME_EXT
            if ((trackSel < trackMax) && (sectSel < 32) && (!diskErr))
#else
        ...
        case  0x0E:
			...
			Z80IntEnFlag = ioData;                // Enable INT_ signal [DEPRECATED]
            Z80IntRx = ioData & 1;                // Enable/disable the Systick IRQ generation
            Z80IntSysTick = (ioData  >> 1) & 1;   // Enable/disable the Serial Rx IRQ generation
          break;
        ...
        case  0x0F:
          ...
          case  0x62: // [DEPRECATED]
            ...
            Z80IntEnFlag = Z80IntRx = ioData ? 1 : 0;
          break;
      ...
      
      {
        ...
        //
        {
          ...
          if (Serial.available() > 0)
          {
            ...
            {
                if ( lastEsc == 0 && kbMap && ioData < 0x80 )
                {
                    ioData = pgm_read_byte( kbMap + ioData );
                }

                // Escape sequences must not be converted
                if ( lastEsc == 0 && ioData == 0x1B )
                {
                    lastEsc = ioData;
                }
                else if ( lastEsc == 0x1B && ( ioData == '[' || ioData == 'O' ) )
                {
                    lastEsc = '[';
                }
                else if ( lastEsc )
  ...
  // Systick interrupt generation is enabled. Check if the INT_ signal must be activated
  {
    ...
    if ((us - timeStamp) > ((long unsigned) (sysTickTime) * 1000))
